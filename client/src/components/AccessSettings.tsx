 ...
 * actions by entity. "People with access": add people by email with
...
export default function AccessSettings({
  ...
  const remove = (entry: DeckShare) => {
    dispatchAction<DeckShare[]>(action('unshare'), {
      ...idInput,
      // A pending invitation has no account behind it, so it is withdrawn
      // by address rather than by user id (SHARE-3).
      ...(entry.pending ? { email: entry.email } : { userId: entry.userId }),
      role: entry.role,
    })
      .then(setShares)
      .catch(() => {
      ...
      <div className="flex flex-col gap-6">
        <div>
          ...
          <ul className="mt-3 flex flex-col gap-1">
            {shares.map(entry => (
              <li
                key={entry.pending ? `invite:${entry.email}` : entry.userId}
                className="flex items-center gap-2 rounded-md border border-slate-200 px-3 py-2 text-sm"
              >
                {entry.pending ? (
                  <span
                    title={t('access.invitedHint')}
                ...
                <select
                  ...
                  aria-label={
                    entry.pending
                      ? t('access.pendingFor', { name: entry.email })
                      : t('access.roleFor', { name: entry.displayName })
                  }
                ...
                >
                  ...
                  <option value="remove">
                    {entry.pending ? t('access.withdraw') : t('access.remove')}
                  </option>
                </select>
              </li>
            ))}
