/**
 * Verifies CLIENT_APP_URL steers only the post-login *landing* (the SPA
 * origin, e.g. Vite on :5173 in dev) while the OAuth callback and token
 * exchange stay on PUBLIC_BASE_URL. The env module is mocked so CLIENT_APP_URL
 * is set without a real .env; the auth service and Google boundary are mocked
