...
- ⬜ Outstanding — **import is what is left**: `TMPL-8` (deriving a style template from a Google Slides presentation; the user-template storage and positioned renderer it was to bring landed with `TMPL-1/4`, so what remains is the importer itself); `EXP-5` (lecture import from Google Slides); `EXP-3` **on the template side** (a template exports as YAML and to Google Slides, but cannot be re-imported from either).
