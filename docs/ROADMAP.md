...
- ⬜ Outstanding — **import is done bar one corner**: `EXP-3` names GitHub as a source alongside an upload and a connected Drive, and a template can be imported from the first two but not yet from a repo. Everything else has landed: `TMPL-8` (a Google Slides deck derives into a style template, consolidating the near-identical slides a hand-built deck is really made of, with a **keep every slide** option for the deck that judgement is wrong for); `EXP-3` **on the template side** (a template exported to YAML re-imports as that template, from an upload or a Drive link, refusing rather than substituting when anything cannot be restored); and `EXP-5` (a lecture from a Google Slides presentation — one read produces both the deck and the template its design became, each slide placed on the layout the design analysis assigned it rather than one guessed again).

  The template work above is **one program across two plans** — [plans/extensible-templates-plan.md](plans/extensible-templates-plan.md) (open slot model, content kinds, dynamic AI contract) and [plans/template-import-plan.md](plans/template-import-plan.md) (Slides import/export, template storage, positioned renderer, consolidation). The first is **delivered**; of the second, storage, the positioned renderer and export have landed, and **import in both directions is the remainder**.

**Phase 3 — Complete — 🟡 in progress.** **Billing is done** — `BILL-1..8` plus `P-8`, ahead of the phase window — as are `TECH-14`, `PLAY-3` and the retention sweep. What has not started is the preflight, GitHub, AI imagery and the hardening pass. (Its other early-delivered items are in Phase 2 above.)
