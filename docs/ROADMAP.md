...
- ⬜ Outstanding: `TMPL-1/4` (library + custom editor); `EXP-1/2/3` (export/import); `AUTH-3/4` (email verify + password reset); `SOC-1/2/3` (voting, browse/search/sort, feeds); `GEN-9` (animated layout transitions); `P-10` (soft delete for all entity data).
...
**Admin / operations (`ADMIN-1..7`, [SPEC §20](SPEC.md#20-administration-operations--moderation)) — 🟡 in progress.** Built opportunistically alongside Phase 2/3; documented in [ADMINISTRATION.md](ADMINISTRATION.md).

- ✅ Done: `ADMIN-1` (email allowlist, `requireAdmin`, ACL bypass); `ADMIN-2` console (user/project/lecture directories + drill-downs); `ADMIN-4` moderation (delete project/lecture/user, ban/unban, password reset — all confirmed + audited); `ADMIN-7` audit log + CSV export; always-on private-lecture viewer; private-**project** view logging (`project.private_view`).
- ⬜ Outstanding: admin deletes as **soft delete** incl. user soft-delete (`P-10` — currently hard cascade); **log private-lecture views** (`ADMIN-3`); **view soft-deleted content** (`ADMIN-6`); **edit any entity's settings** with confirm + log (`ADMIN-5`); **show seed material** on detail pages (`ADMIN-3`); **show db `_id`** on detail pages, **secondary nav** (Users/Projects/Lectures/Logs), **consistent username/project/lecture links** and **"view in product" affordance** across all pages (`ADMIN-2`).
