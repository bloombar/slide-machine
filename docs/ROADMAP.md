...
- ⬜ Outstanding — **lecture import is what is left**: `EXP-5` (lecture import from Google Slides); `EXP-3` **on the template side** (a template exports as YAML and to Google Slides, but cannot be re-imported from either). `TMPL-8` itself is done — a Google Slides deck derives into a style template, consolidating the near-identical slides a hand-built deck is really made of, with a **keep every slide** option for the deck that judgement is wrong for.
