...
- ✅ Done: `QUIZ-1..6`; `SEED-1/2/3` + `EXP-4` (Google); `GEN-7`/`IMG-3`; `GEN-8`; `CAP-4`; `EDIT-2/3` + `TMPL-5`; `AUTH-5`; `SOC-4` (profiles); `TECH-4` (server config); `TECH-9` (billing-provider abstraction); `ADMIN-1`/`2`/`4`/`5`/`6`/`7`/`8`/`9` (allowlist, console, moderation, entity-settings edits, soft-deleted-content viewing + recovery, audit log, settings change log, complimentary plan grants); `P-10` (soft delete for all entity data); `IMG-5` (image attribution capture, on-slide indicator, editable dialog); `EXP-1/2/3` **for decks** (PDF + Google Slides + YAML export, YAML re-import). Delivered early from Phase 3: `GEN-4` (reformat/diarization/narration refine), `PLAY-2` (TTS narration), `EDIT-4`/`5` + `EDIT-6` + `TMPL-7` + `GEN-10` (whiteboard annotation, tools, spoken-transcript editing, layout, generation pause/resume), `TECH-12` (i18n — all five bundles at key parity), `SHARE-2` (translated viewing — adapter, per-deck+locale cache, read-only translated view, locale-aware export; its **usage metering is the one piece still missing**, and belongs to `BILL-3`).
- ⬜ Outstanding: `TMPL-1/4` (library + custom editor); `TMPL-8` (template import from Google Slides, incl. template storage + the positioned renderer); `TMPL-9/10` (open slot & layout model; slot metadata and authoring instructions); `GEN-11` (AI population of declared slots) + `IMG-6` (per-slot image enrichment); `EDIT-7` (editing specialized content slots); `EXP-7/8` (specialized-content export fidelity; slot metadata across Google Slides round trips); `EXP-1/2/3` **on the template side** (deck export/import ships — PDF, YAML, Google Slides, YAML re-import; templates still export YAML download-only and cannot be imported); `EXP-5/6` (lecture import from, and template export to, Google Slides); `AUTH-3/4` (email verify + password reset); `SOC-1/2/3` (voting, browse/search/sort, feeds); `GEN-9` (animated layout transitions); `ADMIN-3` (private-lecture-view logging).
...
**In scope:** [`BILL-1`](SPEC.md#bill-1-subscription-tiers)..[`8`](SPEC.md#bill-8-cap-notifications-email--in-app) + Stripe + metering/enforcement + cost reporting ([`P-8`](SPEC.md#16-privacy-security--compliance)); [`PREP-1`](SPEC.md#prep-1-preflight-concept-extraction)/[`2`](SPEC.md#prep-2-instructor-review--honing)/[`3`](SPEC.md#prep-3-use-of-the-honed-concept-set) (preflight extraction + honing UI) and [`PREP-4`](SPEC.md#prep-4-verbal-interaction-with-the-preflight) (verbal preflight); **GitHub sign-in** ([`AUTH-1`](SPEC.md#auth-1-registration--sign-in-methods)) and **GitHub connect** ([`EXP-4`](SPEC.md#exp-4-connected-accounts-google-drive--github)); [`PLAY-3`](SPEC.md#play-3-narration-in-the-translated-language) (a deck spoken aloud in the language it is being read in, cached like any other narration — the rest of the translation stack, [`TECH-12`](SPEC.md#tech-12-internationalization-i18n--localization) and [`SHARE-2`](SPEC.md#share-2-post-lecture-translated-viewing), shipped early in Phase 2); [`IMG-4`](SPEC.md#img-4-ai-generated-imagery-optional) (AI imagery); [`P-11`](SPEC.md#16-privacy-security--compliance) (**retention purge sweep** — daily job hard-deletes soft-deleted records older than `DELETED_DATA_RETENTION_DAYS`, default 90); full [`P-1..P-9`](SPEC.md#16-privacy-security--compliance) hardening, accessibility (reduced-motion), **100% coverage gate enforced**, and docs.
