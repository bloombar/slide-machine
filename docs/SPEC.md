...
- **Doesn't hijack live capture** — while the instructor is actively drawing during a live session, ongoing speech **folds into the current slide** instead of spawning new ones, and the slide's **layout is held fixed** (both instructed to the model and enforced server-side); explicit new slides (the **+** button or the "new slide" voice command — [CAP-4](#cap-4-voice-commands)) still work. (Design: [DECISIONS.md](DECISIONS.md) "Whiteboard drawings".)
