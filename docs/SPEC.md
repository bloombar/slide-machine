...
- Entries are written through one server module into a dedicated collection and **no API can edit or delete them**. The `Settings changes` page ([ADMIN-2](#admin-2-admin-console)) lists them newest-first, paginated, **filterable by entity kind**, with **CSV export** of the current filter ([P-13](#16-privacy-security--compliance)). The API also filters by entity id (one record's history) and owner id (one account's).
