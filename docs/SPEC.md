...
Registration sends a verification link; unverified accounts have limited capability until verified. Two things are gated: **publishing publicly**, and **sharing with another person** ([SHARE-3](#share-3-share-notifications--invitations)) — sharing sends mail on the account's behalf and hands someone else access, so the account is asked to prove its own address first. The sharing gate is waived where the deployment cannot send mail at all, since confirming is then impossible; the publishing gate is not, because it is about reaching the public rather than about sending mail. Both refusals are distinguishable from a flat "you may not", so the client can offer a fresh confirmation link at the moment it bites rather than sending the user hunting for account settings.
...
- **Which language, on the events that have one.** Reading a lecture in translation ([SHARE-2](#share-2-post-lecture-translated-viewing)) and hearing it narrated ([PLAY-3](#play-3-narration-in-the-translated-language)) record the language on the event, so "how many students read this lecture in each language" is answerable. Nothing else can answer it: the stored translation says which languages a lecture exists in and when each first appeared, but the first viewer of a language creates that entry and everyone behind them is a cache hit against it — in a class of thirty, twenty-eight readings could not be assigned a language by any join between the two. Events with no language leave it blank, which means *not language-specific* and never *English*.
...
- **Admin-only experiment switches** — an allowlisted admin can turn on or off, per lecture, any of the four automatic overrides above (heading-slide promotion, capacity-overflow promotion, whiteboard-slide promotion, and the drawing-in-progress fold into the current slide); the overflow switch also governs [GEN-4](#gen-4-post-lecture-ai-reformat-holistic-regeneration) Refine's box-limit trimming for that lecture. Heading, whiteboard and drawing-in-progress default on (unset = current behaviour); capacity-overflow promotion (and with it Refine's trimming) defaults **off** — an overflowing update lands in place, unclamped, unless an admin turns it on — the switches exist for controlled experimentation, not for instructors. In the lecture settings modal they sit collapsed under an **"Advanced settings"** disclosure, which explains what the switches do and links to the Design tab's per-box character and bullet limits (another way to change how much room a slide has) before showing the four checkboxes.
- The live generation prompt states the **current slide's remaining room outright** — used/max and what's left for bullets and body against the layout's effective limits, saying plainly when the slide is FULL — rather than leaving the model to infer capacity from the layout menu above. A delta update that **resends body text already on the slide** (verbatim, or as one already-said sentence within new material) is ignored for capacity and not re-appended, so a repeated paragraph neither forces a spurious new slide nor gets duplicated.
...
#### GEN-13 Titles for server-promoted slides

The server sometimes overrides the model's decision and turns an **update** into a **new** slide: when the current slide is a heading and the update carries body or bullets, when the update would overflow the layout's budget ([GEN-8](#gen-8-new-slide-vs-update-current)), and when the current slide is a blank whiteboard canvas ([GEN-10](#gen-10-whiteboard-generation-pause--resume)). In each case the model was answering a different question — a delta update returns **only the added material**, and correctly carries no title — so the promoted slide arrives without one.

- **A promoted slide gets a written title, never a transcription.** Deriving one mechanically from the raw phrase reproduces whatever the speaker happened to say at that moment: verbal fillers ("uh", "um"), sentence fragments, and mid-clause openers ("And Uh You Don't Have To") become headings, in a deck whose other titles are model-written. The heading is the most visible text on a slide, so this is the one place a transcription artifact is least acceptable.
- **The model is the source of the title.** Where a promotion is foreseeable the model is asked for a title it would not otherwise supply, so the promoted slide is titled by the same writer as every other slide.
...
- **An address that has not been proven is invited, not granted.** This covers an address with no account *and* one whose account has never confirmed it — registering with an address is not evidence of holding it, so a share to an unconfirmed account waits exactly as one to a stranger does, or registering a colleague's address first would collect their shares. The role is held as a pending invitation on the lecture or project and becomes real access when the address is **proven** — by confirming it ([AUTH-3](#auth-3-email-verification)), by completing a password reset sent to it, or by signing in with Google, which has already verified it. Until it is claimed the invitation grants nothing — an invitation is not access, and no ACL decision reads one.
...
- **Only a confirmed account can share at all**, and notifications are rate-limited per sharer ([AUTH-3](#auth-3-email-verification)). Sharing sends mail carrying text the sender chose — their name and their lecture's title — and grants a stranger access, so it asks the same proof of the sharer that it asks of the recipient. The refusal opens a dialog explaining it and offering a fresh confirmation link, rather than reading like a mistyped address. **Waived where the deployment cannot send mail at all** ([TECH-4](#tech-4-server-configuration)): confirming is then impossible, so the gate would be a permanent refusal rather than a step to take, and there is no relay to protect either.
...
- **Best-effort delivery.** A deployment with no mail relay configured ([TECH-4](#tech-4-server-configuration)) still shares and still invites — including from accounts that could never confirm an address there, per the waiver above; it simply cannot announce any of it. A relay failure on a deployment that *can* send is logged and never fails the share.
...
#### EVAL-7 Lecture view counting

Opening a saved lecture in the viewer ([SHARE-1](#share-1-saved-deck-viewer--permalink)) is recorded, **whether or not the reader is signed in**. It is the denominator the other post-lecture measures were missing: narration ([PLAY-2](#play-2-narration-playback)) and translated reading ([SHARE-2](#share-2-post-lecture-translated-viewing)) are metered and therefore counted, but a lecture nobody opened and a lecture thirty students read straight through look identical without this.

- **Anonymous readers are counted, never identified.** A signed-out reader's opening is recorded with no viewer id, and **nothing is invented to stand in for one** — no cookie, no fingerprint, no visit token. This is the same rule [BILL-7](#bill-7-cost-attribution--admin-cost-reporting) applies to unregistered playbacks and it comes from [§16](#16-privacy-security--compliance): making anonymous readers countable as *people* would trade a reporting convenience for what the privacy commitments promise. So for signed-out readers the answer is "how many openings", and only for signed-in ones "how many people".
- **Reading is free and stays outside billing.** Opening a lecture spends nothing and is recorded in its own collection rather than the cost ledger. The metrics the ledger carries are also the plan's caps ([BILL-3](#bill-3-usage-caps--metering)), so a view recorded there would be an allowance that could one day **refuse to open a lecture** — which no reader should ever meet.
- **One row per opening, and an opening is not a request.** The viewer re-fetches the same lecture to poll for newly retained audio and after a settings change; neither is somebody reading it. The client asks for a view to be recorded when it opens the lecture, so the count cannot be inflated by the page's own housekeeping. A reload or a return visit tomorrow is a new opening, because it is a new reading.
- **Owner separated from audience.** An instructor opening their own lecture is recorded as its owner, so previewing does not inflate the audience it reached — the same line the metered paths already draw.
- **Where it appears.** In the research export ([EVAL-2](#eval-2-de-identified-research-export)) as its own dataset, keyed by study id like every other identity in the bundle.
- **Retention.** Records are kept for a bounded window and swept daily, because this and the cost ledger are the only two collections that grow with *usage* rather than with content ([P-11](#16-privacy-security--compliance)). Rows are **never cascade-deleted** with the lecture they describe — a deleted lecture was still read — so its name is denormalized onto the row.
...
- **DeckView** — `{ id, deckId, deckName, projectId?, projectName?, ownerId, viewerId?, actorKind: 'owner'|'audience', channel, occurredAt }` (one row per lecture opening — EVAL-7; `viewerId` absent for a signed-out reader and never substituted, rows never cascade-deleted, `deckName` denormalized, bounded by `DECK_VIEW_RETENTION_DAYS`)
- **CostEvent** — `{ id, userId, actorUserId?, actorRole: 'instructor'|'student'|'anonymous', projectId?, deckId?, deckName, locale?, service, metric, eventKind: 'billable'|'cached', units, unitPrice, cost, createdAt }` (append-only cost ledger — BILL-7; cost frozen at write time, rows never cascade-deleted, `deckName` denormalized so a deleted lecture keeps its history)
