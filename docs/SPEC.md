...
- **Admin-only experiment switches** — an allowlisted admin can turn off, per lecture, any of the four automatic overrides above (heading-slide promotion, capacity-overflow promotion, whiteboard-slide promotion, and the drawing-in-progress fold into the current slide); the overflow switch also turns off [GEN-4](#gen-4-post-lecture-ai-reformat-holistic-regeneration) Refine's box-limit trimming for that lecture. Unset means on (current behaviour) — the switches exist for controlled experimentation, not for instructors.
