...
- **DeckView** — `{ id, deckId, deckTitle, projectId?, projectName?, ownerId, viewerId?, actorKind: 'owner'|'audience', channel, occurredAt }` (one row per lecture opening — EVAL-7; `viewerId` absent for a signed-out reader and never substituted, rows never cascade-deleted, `deckTitle` denormalized, bounded by `DECK_VIEW_RETENTION_DAYS`)
- **CostEvent** — `{ id, userId, actorUserId?, actorRole: 'instructor'|'student'|'anonymous', projectId?, deckId?, deckTitle, locale?, service, metric, eventKind: 'billable'|'cached', units, unitPrice, cost, createdAt }` (append-only cost ledger — BILL-7; cost frozen at write time, rows never cascade-deleted, `deckTitle` denormalized so a deleted lecture keeps its history)
