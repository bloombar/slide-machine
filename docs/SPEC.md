...
- **Only a confirmed account can share at all**, and notifications are rate-limited per sharer ([AUTH-3](#auth-3-email-verification)). Sharing sends mail carrying text the sender chose — their name and their lecture's title — and grants a stranger access, so it asks the same proof of the sharer that it asks of the recipient. The refusal opens a dialog explaining it and offering a fresh confirmation link, rather than reading like a mistyped address.
