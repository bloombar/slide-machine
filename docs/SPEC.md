...
- **SlideTranslation** — `{ id, deckId, locale: 'en'|'fr'|'es'|'ru'|'zh', perSlide: { slideId: { slots: { [slotName]: translated }, sourceHash? } }, createdAt }` (on-demand slide-content translation cache — SHARE-2; one document per deck + locale, in its own collection. Keyed by slot name like the slide itself ([TMPL-9](#tmpl-9-open-slot--layout-model)), and covering the slots whose content is prose — a program listing or a formula is not translated. `sourceHash` fingerprints the slide content an entry was translated from, so an edited slide re-translates on its next view and its neighbours do not. A derived cache with no authored content in it, so the delete cascade removes it outright rather than tombstoning it — a restored deck simply re-translates.)
